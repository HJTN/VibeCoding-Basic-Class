...
무료 계정만으로는 CLI 로그인이 되지 않습니다.

| 구분 | 필요한 계정 | 요금제 | 비고 |
| :---: | :--- | :--- | :--- |
| Antigravity<br>(2.0·IDE·CLI) | Google(Gmail) 계정 | 무료 요금제로 시작 가능 | 일일 사용량 제한 존재,<br>Google AI Pro/Ultra로 한도 확장 |
| Antigravity SDK | Gemini API Key<br>또는 GCP 계정 | 사용량 기반 과금 | 선택 사항(심화 실습용) |
| Claude Code CLI | Anthropic 계정 | Pro 이상 유료 구독 | 무료 요금제는 사용 불가 |
| Codex CLI | ChatGPT 계정 | Plus 이상 유료 구독 | API Key 방식도 가능 |

- 구독 결제가 부담되는 경우, **Antigravity만 먼저 설치**하셔도 1차시 참여가 가능합니다.
- 유료 구독 여부는 개인 선택이므로, 운영진이 결제를 권유하지 않습니다.
...
7차시에서 본격적으로 다루므로 **가장 먼저 설치**해 주세요.
...
네 도구는 모두 **동일한 에이전트 하네스(Agent Harness)** 위에서 동작합니다.  
따라서 플러그인, 스킬, 설정을 서로 공유하며 같은 프로젝트 폴더를 함께 사용합니다.  
어떤 도구로 작업하든 결과 파일은 동일한 위치에 저장됩니다.
...
- 1차시 전까지 **Antigravity 2.0**과 **Antigravity IDE**를 모두 설치해 주세요.
- 두 앱은 함께 설치해도 서로 충돌하지 않습니다.
- **CLI**와 **SDK**는 선택 사항입니다!
- 기존에 Antigravity 1.x를 쓰셨다면 자동 업데이트 안내를 통해 2.0을 추가 설치합니다.
...
2. **Antigravity 2.0** 항목에서 본인의 운영체제와 아키텍처를 선택합니다.
3. 내려받은 설치 파일을 실행하여 설치를 완료합니다.
4. 설치가 끝나면 Antigravity 2.0을 실행합니다.

### 2) 최초 실행 설정

최초 실행 시 설정 마법사가 순서대로 나타납니다.  
아래 순서대로 진행하시면 됩니다.
...
1. 노트북에 작업용 폴더를 하나 만듭니다. (예: `C:\Users\사용자명\vibe-coding`)
...
- 2.0에서 에이전트가 코드를 수정하면 IDE 화면에 즉시 반영됩니다.
- 작업은 2.0에 맡기고, 검토는 IDE에서 하는 방식을 권장합니다.

### 4) 확장 프로그램 관련 유의 사항

Antigravity IDE는 VS Code 마켓플레이스가 아닌 **OpenVSX 레지스트리**를 사용합니다.  
따라서 일부 VS Code 확장 프로그램은 검색 결과에 나타나지 않습니다.

- 검색되지 않는 확장은 `.vsix` 파일을 내려받아 설치합니다.
- 확장 패널에서 `Install from VSIX...`를 선택한 뒤 파일을 지정합니다.
- VS Code 전용 API에 의존하는 확장은 정상 동작하지 않을 수 있습니다.

## 마. Antigravity CLI 설치 (선택)

Antigravity CLI는 터미널에서 동작하는 에이전트 도구입니다.  
Go 언어로 만들어 실행 속도가 빠르며, 키보드만으로 조작합니다.  
...
- 프롬프트가 `PS C:\`로 시작하면 PowerShell입니다.
- 프롬프트가 `C:\`로 시작하면 CMD입니다.
